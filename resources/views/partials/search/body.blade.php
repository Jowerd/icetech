    ...
    <div class="d-none d-md-block bg-white p-3 rounded shadow-sm sticky-top" style="top: 20px;">
        ...
        <form method="GET" action="{{ route('products.search') }}">
            ...
            @if(request()->hasAny(['min_price', 'max_price', 'condition', 'country']))
                <a href="{{ route('products.search') }}?query={{ $query }}" class="btn btn-outline-secondary w-100 mt-2 btn-sm">
    ...
    @else
        <div class="d-flex justify-content-between align-items-center mb-3">
            ...
            <div class="d-flex gap-2">
                <button class="btn btn-sm btn-outline-secondary view-mode" data-view="grid" title="ბადისებური ხედი">
                    <i class="bi bi-grid"></i>
                </button>
                <button class="btn btn-sm btn-outline-secondary view-mode" data-view="list" title="სიის ხედი">
...
<div class="offcanvas offcanvas-start" tabindex="-1" id="filterOffcanvas">
    ...
    <div class="offcanvas-body">
        <form method="GET" action="{{ route('products.search') }}">
            <input type="hidden" name="query" value="{{ $query }}">
            {{-- იგივე ფილტრები გადმოწერე ზემოდან --}}
            
            <div class="mb-3">
                <label class="form-label fw-medium">ფასი</label>
                <div class="input-group mb-2">
                    <span class="input-group-text">მინ ₾</span>
                    <input type="number" name="min_price" class="form-control" value="{{ request('min_price') }}">
                </div>
                <div class="input-group">
                    <span class="input-group-text">მაქს ₾</span>
                    <input type="number" name="max_price" class="form-control" value="{{ request('max_price') }}">
                </div>
            </div>
            ...
            <div class="mb-3">
                ...
                <div class="d-flex flex-column gap-2">
                    @foreach ([
                        ...
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="condition" id="mCondition_{{ $value ?: 'all' }}" value="{{ $value }}" {{ request('condition') == $value ? 'checked' : '' }}>
                            <label class="form-check-label" for="mCondition_{{ $value ?: 'all' }}">{{ $label }}</label>
                        </div>
                    @endforeach
                </div>
            </div>
